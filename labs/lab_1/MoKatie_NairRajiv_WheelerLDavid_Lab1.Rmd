...
The dataset used in the examination of probability of O-ring failure in Challenger's previous space shuttle launches by Dalel et al. (1989) included 23 rows and 5 variables. The `Flight` column is made up of unique identifiers representing the flight number of a launch. The `Temp` column contains information about the temperature (°F) at launch, and ranges between 53 and 81 °F for the 23 data points. Notably, the temperature on the day of the space shuttle Challenger accident, 31 °F, was much lower than lowest previously recorded launch temperature provided in this dataset. The `Pressure` column contains information about the combustion pressure (psi) at launch, and takes on only 3 values among the 23 data points: 50 100, or 200. The `O.ring` column represents the number of primary field O-ring failures in a launch, which is our outcome of interest. For this dataset, the `O.ring` column contains only the values 0, 1, or 2. The `Number` column contains the total number of primary field O-rings, which is 6 for all launches (three each for the two booster rockets). Each of the 5 variables were treated as `int` classes. 
...
# Cross tabulation
three = ggplot(as.data.frame(table(df)),
...
In our bivariate analysis, we compared the relationship between our two putative predictors, temperature and pressure, and our outcome variable, the number of primary O-ring failures.

```{r, fig.width=6, fig.height=3, echo=F}
four = ggplot(df, aes(x=Temp, y=as.factor(O.ring), colour=O.ring>0)) +
...
=======
  labs(title=expression(~bold('Figure 2:')~'O-ring failure as a function of temperature'),
>>>>>>> e2b1c5602b472803bff302c28a0ad830774d960f
       color="Observations:")+
  theme(legend.position = "top") +  
  theme(plot.title = element_text(hjust=0)) + 
  theme(
...
```{r, fig.width=6, fig.height=3, echo=F}
<<<<<<< HEAD
five = ggplot(df, aes(x=Pressure, y=as.factor(O.ring), colour=O.ring>0)) +
...
ggplot(df, aes(x=Pressure, y=as.factor(O.ring), colour=O.ring>0)) +
  geom_jitter(size=4, pch=20, alpha=3/4, stroke=3, position=position_jitter(w=1, h=0.2))+
  scale_color_manual(labels = c("ignored by NASA","used by NASA"),
                                values = rev(c("black","olivedrab"))) +
...
Based on the description in the paper, we do not anticipate there to be any meaningful relationship between temperature and pressure as they are both arbitrarily determined. Pressure is a condition set by the test procedure, which was first at 50 psi, then progressively updated to 100 psi and 200 psi. Temperature is merely a condition at the time of launch. The correlation between the two variables, 'r round(cor(df$Temp, df$Pressure),3)' supports our understanding.


```{r, fig.width=7, fig.height=4, echo=F, results='hide'} 
six = ggplot(df, aes(x=Flight, y=as.factor(O.ring), colour=O.ring>0)) +
...
stargazer(model_a, model_a.1, type="text")
```
...
```{r}
b <- model_a$coefficients

curve(exp(b[1]+b[2]*x + b[3]*200)/(1+exp(b[1]+b[2]*x + b[3]*200)), from=30, to=100, xlab="Temperature", ylab="Probability of failure", col="blue")
text(x=40, y=0.7, label="Binomial", col = "Blue", title("Binomial vs Binary Logistic Regression (At Pressure=200)"))


b <- model_a.1$coefficients

curve(exp(b[1]+b[2]*x+ b[3]*200)/(1+exp(b[1]+b[2]*x+ b[3]*200)), from=30, to=100, xlab="Temperature", ylab="Probability of failure", add=TRUE, col="black")
...
model_b <- glm(O.ring/Number ~ Temp, family = binomial(link = logit),
...
plot(df$Temp, df$O.ring/df$Number,
     ...
     ylim=c(0,1))
# Title
title(expression(bold("Figure 4:")~pi~"as a function of temperature (°F)"), adj=0)
# Betas
...
plot(df$Temp, df$O.ring/df$Number,
     ...
     ylim=c(0,1))
# Title
title(expression("with confidence intervals"), adj=0)
# Betas
b0 <- model_b$coefficients[1]
b1 <- model_b$coefficients[2]
curve(expr = exp(b0+b1*x)/(1+exp(b0+b1*x)), add=T)
...
plot(df$Temp, df$O.ring,
     ...
     ylim=c(0,6))
# Title
title(expression("expected number of failures as a function of temperature (°F)"), adj=0)
## Binomial betas
# Betas
b0 <- model_b$coefficients[1]
b1 <- model_b$coefficients[2]
...
hist(results[,1],
     ...
     main="31 (°F)", adj=1)
# Title
title(expression(bold("Figure 5:")~"Probability of failure when temperature:"), adj=0)
...
plot(model_4$fitted.values, model_4$residuals,
     ...
     ylab="Residuals")
# Title
title(expression(bold("Figure 6:")~"Residual vs. fitted values &"), adj=0)
