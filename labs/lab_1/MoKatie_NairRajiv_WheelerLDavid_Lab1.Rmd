...
ggplot(as.data.frame(table(df)),
   ...
   labs(title="Figure 1: Distribution of O-ring failures")+
...
ggplot(df, aes(x=Temp, y=as.factor(O.ring), colour=O.ring>0)) +
  ...
  theme_classic() +
  theme(text = element_text(size = 20)) +
  labs(title="Figure 2: O-ring failure as a function of temperature (°F)",
       color="Observations:")+
  theme(legend.position = "top") +  
  theme(plot.title = element_text(hjust=0)) + 
  theme(
...
ggplot(df, aes(x=Pressure, y=as.factor(O.ring), colour=O.ring>0)) +
  ...
  theme_classic() +
  theme(text = element_text(size = 20)) +
    labs(title="Figure 3: O-ring failure as a function of pressure",
       color="Observations:")+
  theme(legend.position = "top") +
  theme(plot.title = element_text(hjust=0)) + 
  theme(
...
ggplot(df, aes(x=Flight, y=as.factor(O.ring), colour=O.ring>0)) +
  ...
  theme_classic() +
  theme(text = element_text(size = 20)) +
    labs(title="Figure 4: O-ring failure as a function of flight number/time",
       color="Observations:")+
  theme(legend.position = "top") +
  theme(plot.title = element_text(hjust=0)) + 
  theme(
...
$$ logit\left(\cfrac{O.ring}{Number}\right)= 2.520 - 0.098\text{Temp} + 0.008\text{Pressure} $$ 
...
# model_3b = glm(O.ringBinary ~ Temp,
#                family = binomial(link = logit),
#                data = df)
# 
# # Number of bootstrap samples
# Bn = 1000                
# # E.m = empty B.n x n matrix for estimates
# E.m = matrix(nrow = Bn, ncol = length(coef(model_3b)) + 2,
#            dimnames = list(paste("Sample",1:Bn), 
#                            c(names(coef(model_3b)), "t.31", "t.72")))
# # For each bootstrap sample
# for(i in 1:Bn){
#     # sample credit data with replacement
#     bs.df = df[sample(x = 1:nrow(df),
#                           size = nrow(df),
#                           replace = TRUE), ]
#     
#     # Fit model on the boostrapped sample
#     bs.logit = glm(model_3b$formula,
#                      family = binomial(link = "logit"),
#                                           data=bs.df)
#     # Store  coefficients
#     E.m[i,] = coef(bs.logit)
#     
#     # Confidence interval code from Rajiv
#     # Temp = 31
#     temp.31.data <- data.frame(Temp=31)
#     # Store confidence interval in column 3
#     E.m[i, 3] <- predict(object = bs.logit , newdata = temp.31.data,
#                         type = "response")
# 
#     # Temp = 72
#     temp.72.data <- data.frame(Temp=72)
#     # Store confidence interval in column 4
#     E.m[i, 4] <- predict(object = bs.logit , newdata = temp.72.data,
#                         type = "response")
# }
# 
# paste("For 31 degrees CI =")
# quantile(E.m[,3], probs=c(0.05, 0.95))
# paste("For 72 degrees CI =")
# quantile(E.m[,4], probs=c(0.05, 0.95))
...
Here we see that an increase in temperature changes the odds of failure by 0.89 times for each degree increase. In other words, a unit decrease in temperature increases the odds of failure increases by 1.11 times for each unit temperature decrease.

In terms of probability of failure, as is seen in the chart of probability of failure vs temperature in part 3c, we clearly see an increase in the probability of failure with lower temperature. For temperature going from 50F, 40F and 30F, we see probability of failure $\pi$ values of  0.33, 0.61 to 0.83, which clearly shows that lower temperatures lead to a higher probability of failure.
