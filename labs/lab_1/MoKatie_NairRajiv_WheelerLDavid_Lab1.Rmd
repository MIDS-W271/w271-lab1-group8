...
plot_oring = ggplot(as.data.frame(table(df)),
   ...
   labs(title=expression(~bold('Figure 1:\n')~' Distribution of O-ring failures, '))+
...
## TO DO: add blurb about **Figure 2 (c)** 
...
## TO DO: add blurb about **Figure 3** 
...
# Estimate a binary logistic regression model
...
model_b <- glm(O.ring/Number ~ Temp,
               ...
               data = df)
# Summary of coefficients, standard errors and p-values
stargazer(model_b,model_b2,  type="text")
```
...
Using the parametric bootstrap method to estimate confidence interval, we again found that the confidence interval for 31 degrees F is quite wide at `r round(CI.31[1],4)` and `r round(CI.31[2],4)`, due to the lack of data points at low temperatures. The confidence interval estimated for 72 degrees F is much tighter, at `r round(CI.72[1],4)` and `r round(CI.72[2],4)`. 
...
Out of the 1000 simulated datasets, we plotted a histogram of their probability of failures. The confidence intervals were taken as the 5th and 95th percentile of the distributions. The probability of failures for 31 degrees F took on the full range of 0 and 1, and had a left skew. In contrast, the probability of failures for 72 degrees F were more concentrated at the left end towards 0. 
