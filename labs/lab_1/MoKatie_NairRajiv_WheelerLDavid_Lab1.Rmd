...
pred.func <- function(seq, data) {
  d <- data[sample(1:nrow(data), 23,replace=TRUE),]  

  mod <- glm(O.ring/Number ~ Temp,
               family = binomial(link = logit),
               weights = Number,
               data = d)
...
ci_list <- lapply(1:1000, pred.func, df)

results <- do.call(rbind,ci_list)
