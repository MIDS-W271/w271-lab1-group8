...
str(df)
```
  Note that the data are a 23 x 5 dimensional data.frame. Each of the 5 variables are treated as `int` classes. For now, this is OK, however, we may transform O.ring into a binary factor for subsequent analyses.`Pressure` is treated as integer but only has `r length(unique(df$Pressure))`. Thus, we will treat it as a factor *on the fly*.
...
- Conversely, the binary model does 
...
- BinomialLogistic regression
```{r, warning=F}
# Estimate a binomial logistic regression model
model_a<- glm(O.ring/Number ~ Temp + factor(Pressure),
               family = binomial(link = logit),
               weights = Number,
               data = df)
# Estimate a binary logistic regression model
model_bin <- glm(O.ringBinary ~ Temp + factor(Pressure),
               family = binomial(link = logit),
               data = df)

# Summary of coefficients, standard errors and p-values
stargazer(model_a,model_bin,
          type="text")
```
...
predict(object = model_b,
        ...
        type = "link")
# Predict response 
pi.hat = predict(object = model_b, 
        newdata = predict.data,
        type = "response")
# Confidence interval
c(CI_lower_pi[[1]], CI_higher_pi[[1]])
# Tabulate results
round(data.frame(Temp = 31, 
                 pi.hat,
                 pi.lower.CI = CI_lower_pi[[1]],
                 pi.higher.CI = CI_higher_pi[[1]]), 
      3)
```

- Thus, the probability of O-ring failure is `r pi.hat` with a very wide confidence interval of `r c(CI_lower_pi[[1]], CI_higher_pi[[1]])`.
...
- For `temp^2`, $-2log(\Lambda)$ = `r anova(model_b, model_3f, test="Chisq")[2,4]` and the *p*-value of $P(\chi^2_1 >$ `r anova(model_b, model_3f, test="Chisq")[2,4]`) = `r anova(model_b, model_3f, test="Chisq")[2,5]` > $\alpha = 0.05$. Thus, `temp^2` is not important, when we hold `temp` constant.
