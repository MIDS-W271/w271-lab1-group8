...
lapply(df, unique)
```
  - Note that O.ring includes 0,1 and 2.
...
- **Problems that might arise from the removal of this variable include**
...
pred.func <- function(data, indices) {
...
results <- boot(data=df, statistic=pred.func,
   R=1000)

paste("For 31 degrees CI =")
quantile(results$t[,1], probs=c(0.05, 0.95))
paste("For 72 degrees CI =")
quantile(results$t[,2], probs=c(0.05, 0.95))
```
...
```{r}
# Estimate logistic regression model
model_3f <- glm(Failure ~ Temp + I(Temp^2),
               family = binomial(link = logit),
               data = df_ind)
# Summary of coefficients, standard errors and p-values
summary(model_3f)
# Estimate logistic regression model
model_3f.2 <- glm(O.ring/Number ~ Temp + I(Temp^2),
               family = binomial(link = logit),
               weights = Number,
               data = df)
# Summary of coefficients, standard errors and p-values
...
```{r}
# Estimate model
model_4 = lm(formula = Failure ~ Temp + I(Temp^2),
             data = df_ind)
# Summary
summary(model_4)
# Estimate model
model_4.b = lm(formula = O.ring/Number ~ Temp + I(Temp^2),
             data = df)
# Summary
summary(model_4.b)
```

- Explaination of the model results:
...
- Assess the validity of the model assumptions:
  ...
  - **Assumption MLR.3 (No Perfect Collinearity):**
    - From below we can see that the two explanatory variables- `Temp` and `Temp`$^2$- are nearly perfectly correlated, with a correlation of `r cor.test(df$Temp, df$Temp^2)$estimate`.
```{r warning=FALSE, message=FALSE}
# Scatterplot
plot(jitter(df$Temp, 0.001), jitter(df$Temp^2,1),
     pch=20, cex=2,
     xlab="Temperature", ylab=expression(paste("Temperature"^"2")),
     data=df)
```
