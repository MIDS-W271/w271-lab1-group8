...
ggplot(df, aes(x=Temp, y=O.ring, colour= O.ring>0)) +
  geom_jitter(size = 10, alpha = 0.75, height=.05, width=.3) +
...
E.m = matrix(nrow = Bn, ncol = length(coef(model_3b)) + 2,
           dimnames = list(paste("Sample", 1:Bn), 
                           c(names(coef(model_3b)), "T31", "T72")))

# For each sample
for(i in 1:Bn){
    # sample all rows and columns of interest with replacement
    BS.df = df[sample(x = 1:nrow(df), 
                          size = nrow(df),
                          replace = TRUE),
                   c(2,6) ]
    # Estimate the model with the boostrapped sample as data
    BS.model = glm(model_3b$formula, 
                     family = binomial(link = "logit"),
                     data = BS.df)
    # store the coefficients
    E.m[i,] = coef(BS.model)
    
    # Confidence interval codel from Rajiv
    # Temp = 31
    temp.31.data <- data.frame(Temp=31)
    E.m[i, 3] <- predict(object = BS.model, newdata = temp.31.data,
                        type = "response")

    # Temp = 72
    temp.72.data <- data.frame(Temp=72)
    E.m[i, 4] <- predict(object = BS.model, newdata = temp.72.data,
                        type = "response")
    
    return (c(temp.31, temp.72))
}
