...
The dataset used in the examination of probability of O-ring failure in Challenger's previous space shuttle launches by Dalel et al. (1989) included 23 rows and 5 variables. The Flight column is made up of unique identifiers representing the flight number of a launch. The Temp column contains information about the temperature (F) at launch, and ranges between 53 and 81 degrees for the 23 data points. The Pressure column contains information about the combustion pressure (psi) at launch, and takes on only 3 values among the 23 data points: 50 100, or 200. The O.ring column represents the number of primary field O-ring failures in a launch, which is our outcome of interest. For this dataset, the O.ring column contains only the values 0, 1, or 2. The Number column contains the total number of primary field O-rings, which is 6 for all launches (three each for the two booster rockets). Each of the 5 variables were treated as `int` classes. 

Because the O.ring column contains more than 2 outcomes, we also created a new factor variable to binarize the dependent variable. This new column, O.ringBinary, would have an outcome of 1 if the number of primary O-ring failures in a launch was 1 or greater.
 
```{r}
# Create binary column for O.ring failture
df$O.ringBinary = ifelse(test = df$O.ring > 0, yes = 1, no = 0)
...
To ensure that the data do not contain anomolous observations, we next inspected the unique values for each variables. The values for each variable seem reasonable according to their description and we do not observe any top coded or bottom coded values. We also noted that there are no missing values in any of the variables.

**Univariate Analysis**

The three variables of interest for modeling purposes in the paper by Dalel et al. were temperature, pressure, and number of O-ring failures. 
...
From the histogram, the temperature is seen to peak at 70 degrees with 4 observations and tapered off on either side. Based on the plot, the distribution seems to most closely resemble a normal distribution.
...
Pressure is seen to only take on 3 unique values, with the most observations having a pressure of 200. Though it is difficult to say with such few unique values, the distribution seems to most closely resemble a bimodal distribution.
...
#not sure if we can make this assumption that flight number=time? made this code block hidden
```
...
- The independence assumption is necessary for the binomial model becuase it allows us to use the binomial probability mass function. Moreover, the assumption is likely violated since the failure of 1 0-ring might expose other 0-rings to damaging conditions and or might exert stress on the remaining 0-rings.

- Conversely, the binary model does suffer from this assumption since flights are equivalent to trials and we assume that each flight is indepedent.
...
- Above, we tested $H_0: \beta = 0$ vs $H_A:\beta \neq 0$, for `Pressure`.The $-2log(\Lambda)$ = `r Anova(model_a, test='LR')[2,1]` and the *p*-value of $P\left(\chi^2_1 > \right.$ `r Anova(model_a, test='LR')[2,1]`) = `r Anova(model_a, test='LR')[2,3]` > $\alpha$ 0.05. 
...
plot(df$Temp, df$O.ring/df$Number,
     ...
     ylim=c(0,1))
# Title
title(expression(bold("Figure 6:")~pi~"as a function of temperature (°F)"), adj=0)
# Betas
...
plot(df$Temp, df$O.ring,
     ...
     ylim=c(0,6))
# Title
title(expression(bold("Figure 7:")~"Expected number of failures as a function of temperature (°F)"), adj=0)
## Binomial betas
# Betas
b0 <- model_b$coefficients[1]
b1 <- model_b$coefficients[2]
...
plot(df$Temp, df$O.ring/df$Number,
     ...
     ylim=c(0,1))
# Title
title(expression(bold("Figure 8:")~pi~"as a function of temperature (°F)"), adj=0)
# Betas
b0 <- model_b$coefficients[1]
b1 <- model_b$coefficients[2]
curve(expr = exp(b0+b1*x)/(1+exp(b0+b1*x)), add=T)
...
data.frame(temperature=31,estimate=predict.pi, lower=CI_lower_pi[[1]], upper=CI_higher_pi[[1]])
```
...
- The assumptions needed to apply the inference procedures are:
  ...
  - We are assuming that the observations are independent. i.e. The failure of one O-ring does not increase the odds of the other to fail
...
Create new column o.ring2 and initialize to o.ring. This column will be updated in each iteration.
Do the following steps in a loop (1..100):
* sample 23 points from dataset with replacement
* fit the model using the above points
* Calculate Z using intercept + coefTemp * Temp This results in a vector of 23 values
* Calculate pi using exp(z) / (1+exp(z)) (vector of size 23)
* Generate vector of size 23 of binomials 0-6 using the built in rbinom function by passing in the pi's generated in prior step.
* replace o.ring2 with above generated binom values, updating only the row numbers corresponding to sample.
* run predictions for 31 and 72 degrees and save results

At the end, the CI will be the 0.05 and 0.95 quantile of the accumumated results.
...
# For each sample in 1:n
for (s in 1:100){
  # Collect sample of size 23 from the whole data frame with replacement
  ...
  # Estimate model with bootstrap sample data
  ...
  if (s==1){
    # Z is the the beta0 + beta1X
    z = mod$coefficients["(Intercept)"] + mod$coefficients["Temp"] * d$Temp
  
    # Pi is the binary Logistic Regression
    pi = exp(z)/(1+exp(z))
  }  
  # Generate new pi values from pi values generated with bootstrap samples
  pi.O.ring <- rbinom(n=23, # sample size
...
hist(results[,1],
     ...
     main="31 (°F)", adj=1)
# Title
title(expression(bold("Figure 9:")~"Probability of failure when temperature:"), adj=0)
...
par(mfrow=c(2,1))
# Histogram for temp = 31
hist(results[,1], breaks=50, col="lightskyblue1",
     main="Bootstrapped confidence intervals for temperature (°F)= 31")

# Histogram for temp = 72
hist(results[,2], breaks=50, col="orange",
     main="Bootstrapped confidence intervals for temperature (°F)= 72")
```
...
- For `temp^2`, $-2log(\Lambda)$ = `r anova(model_b, model_3f, test="Chisq")[2,4]` and the *p*-value of $P\left(\chi^2_1 > \right.$ `r anova(model_b, model_3f, test="Chisq")[2,4]`) = `r anova(model_b, model_3f, test="Chisq")[2,5]` > $\alpha = 0.05$. Thus, `temp^2` is not important, when we hold `temp` constant.
...
```{r warning=FALSE}
# Estimate model
model_4 = lm(formula = O.ring/Number ~ Temp, data = df, weights = Number)
# Summary
...
plot(model_4$fitted.values, model_4$residuals,
     ...
     ylab="Residuals")
# Title
title(expression(bold("Figure 10:")~"Residual vs. fitted values &"), adj=0)
...
qqline(df$O.ring,
       col="red", lwd=2)
# Title
title(expression("Normal q-q plot"), adj=0)
```
...
Our final model model is $$logit\left(\hat\pi\right) = 5.085 - 0.116 Temp$$
...
Here we see that an increase in temperature changes the odds of failure by 0.89 times for each degree increase. In other words, a unit decrease in temperature increases the odds of failure by 1.12 times for each unit temperature decrease.
